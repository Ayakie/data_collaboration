...
num_users_list = np.arange(2, 31, 2).tolist()

if __name__ == '__main__':
    
    fl_score = np.zeros((args.repeat, 3, len(num_users_list)))
    centr_score = np.zeros((args.repeat, 3, len(num_users_list)))
    ind_score = np.zeros((args.repeat, 3, len(num_users_list)))
    dc_score = np.zeros((args.repeat, 2, 3, len(num_users_list)))
    
    for r in range(args.repeat):
        print('Repeat: ', r+1)
        random.seed(r)

        for n, user in enumerate(num_users_list):

            print('Number of user: ', user)
            X_train, X_test, X_anc, label_train, label_test, user_list = get_dataset(args, num_users=user)
            ...
            anc_gan = make_anchors(X_anc, anc_type=args.anc_type) # 'gan' or 'raw'
            ...
            X_dc1, X_test_dc1, _ = data_collaboration(Div_data_gan, ir_method, args)
            X_dc2, X_test_dc2, _ = data_collaboration(Div_data_random, ir_method, args)
    ...
    xlabel = f'Number of user ({args.ndat} data)'
    get_3col_plt_2dc(centr_score, ind_score, dc_score, fl_score, args, xlabel, num_users_list)
